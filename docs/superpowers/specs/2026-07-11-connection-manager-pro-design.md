...
**Windows caveat**: `"symlink": true` requires either Developer Mode or an elevated (admin) shell to create real symlinks; without one, Composer transparently falls back to copying the files into `vendor/` instead of linking them. On a normal (non-elevated) Windows shell, that means edits to Core aren't picked up until `composer install` is re-run in the Pro repo — not truly "live" like a symlink would be, just faster than a full tag/push/update round-trip. Worth confirming Developer Mode is enabled locally if the copy-per-`composer install` step becomes annoying.
